...
#' @usage betareg.stan(des, y, model="beta.ols", link="logit", covmat=NULL, 
...
#' @param y One of "beta.ols" or "beta.mm" for a basic beta regression model or beta mixed model, respectively; defaults to "beta.ols". 
#' @param model A vector of response variable observations. 
...
#' @export
betareg.stan = function(des, y, model="beta.ols", link="logit", covmat=NULL, iter=2000, chains=4, coef.u=0, 
  ...
  if(!is.null(covmat) & model == "beta.ols") stop("Covariance matrix supplied for 'beta.ols' model. Choose alternative model or don't supply covmat")
  ...
  # Set model choice
  model.choice = switch(model, beta.ols = 1, beta.mm = 2, stop("'model' given to 'betareg.stan' must be one of 'beta.ols' or 'beta.mm'"))
  # Prep stan data
  ...
  stan.dat$Cp = if(model=="beta.ols") diag(length(y)) else covmat
  # Fit the model 
